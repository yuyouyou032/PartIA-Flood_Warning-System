...
def test_stations_by_distance(mock_stations):
    ...
    for station, distance in result:
        ...
        assert distance >= 0
def test_stations_within_radius(mock_stations):
...
def test_rivers_with_station(mock_stations):
    result = rivers_with_station(mock_stations)
    assert isinstance(result, set)
    assert result == {"River A", "River B", "River C"}
def test_stations_by_river(mock_stations):
...
def test_rivers_by_station_number(mock_stations):
